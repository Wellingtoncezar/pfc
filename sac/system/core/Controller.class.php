	...
	*/
	public function saveModules(){
		...
		foreach ($rota as $modulo)
		{
			...
			}else
			{
				...
				if($this->rowCount() > 0)
				{
					...
					$this->query('SELECT id_modulo FROM modulos ORDER BY id_modulo DESC LIMIT 1');
					$res = $this->result();
					$this->_idModulo = $res['id_modulo'];
		...
		}else
		{
			...
			{
				//retorna o id do módulo inserido
				$this->query('SELECT id_pagina FROM paginas ORDER BY id_pagina DESC LIMIT 1');
				$res = $this->result();
				$this->_idPagina = $res['id_pagina'];
	...
	public function saveAction(){
		...
		}else
		{
			...
			{
				//retorna o id do módulo inserido
				$this->query('SELECT id_action FROM actions ORDER BY id_action DESC LIMIT 1');
				$res = $this->result();
