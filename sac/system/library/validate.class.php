...
class validate{
	...
	public static function ip($valor)
	{
	...
	{
		if(!filter_var($valor, FILTER_SANITIZE_STRING))
			return false;
		else
			return true;
	}
	...
	public static function data($valor, $limitador = '-',$formato = 'dd-mm-yyyy')
	{
		$data = explode($limitador,$valor); // fatia a string $dat em pedados, usando / como referência
		if(count($data) == 3)
		{
			$formato = explode($limitador,$formato);
			if($formato[0] == 'dd')
				$d = $data[0];
			else
			if($formato[1] == 'dd')
	...
	public static function cpf_cnpj($valor)// VALIDAR CPF NO SEGUINTE FORMATO: XXX.XXX.XXX-XX
	{
